...
void trdclass_halld24::Loop() {
  ...
  f125_el_x = new TH1F("f125_el_x","GEM-TRD f125 Peak X Amp ; ADC Amplitude ; Counts ",100,0.,4096);
  f125_el_y = new TH1F("f125_el_y","GEM-TRD f125 Peak Y Amp ; ADC Amplitude ; Counts ",100,0.,4096);
  f125_el_Xmax = new TH1F("f125_el_Xmax","GEM-TRD f125 Max X Amp ; ADC Amplitude ; Counts ",100,0.,4096);
  f125_el_Ymax = new TH1F("f125_el_Ymax","GEM-TRD f125 Max Y Amp ; ADC Amplitude ; Counts ",100,0.,4096);
  f125_el_xch_max = new TH1F("f125_el_xch_max","GEM-TRD f125 X Channel of Max Amp ; GEMTRD Channel (X) ; Counts ",128,-0.5,127.5);
	f125_el_ych_max = new TH1F("f125_el_ych_max","GEM-TRD f125 Y Channel of Max Amp ; GEMTRD Channel (Y) ; Counts ",528,-0.5,527.5);
	
  //-- Prototype ADC Amplitudes in Time (2D)
  f125_yamp2d = new TH2F("f125_yamp2d","GEM-TRD Amp. in Time ; Time Response (8ns) ; GEMTRD Channel (Y) ",250,0.5,250.5,528,-0.5,527.5);
  f125_xamp2d = new TH2F("f125_xamp2d","GEM-TRD Amp. in Time ; Time Response (8ns) ; GEMTRD Channel (X) ",250,0.5,250.5,128,-0.5,127.5);
  
  //-- GEM Trackers
  hgemtrkr_peak_xy = new TH2F("hgemtrkr_peak_xy","GEM-TRKR Peak X-Y Correlation (mm); Peak X [mm]; Peak Y [mm] ",512,-0.5,511.5,128,-0.5,127.5);
	hgemtrkr_2_peak_xy = new TH2F("hgemtrkr_2_peak_xy","GEM-TRKR 2 Peak X-Y Correlation (mm); Peak X [mm]; Peak Y [mm] ",512,-0.5,511.5,128,-0.5,127.5);
	hgemtrkr_max_x = new TH1F("hgemtrkr_max_x","GEM-TRKR1 Max Amp per Event; Max ADC Amplitude ",4096,0.5,4096.5);
	hgemtrkr_2_max_x = new TH1F("hgemtrkr_2_max_x","GEM-TRKR2 Max Amp per Event; Max ADC Amplitude ",4096,0.5,4096.5);
	hgemtrkr_double_y = new TH2F("hgemtrkr_double_y","GEM-TRKR1 & GEM-TRKR2 Peak Y Correlation (mm); GEMTR1 Peak Y [mm]; GEMTR2 Peak Y [mm] ",128,-0.5,127.5,128,-0.5,127.5);
	hgemtrkr_double_x = new TH2F("hgemtrkr_double_x","GEM-TRKR1 & GEM-TRKR2 Peak X Correlation (mm); GEMTR1 Peak X [mm]; GEMTR2 Peak X [mm] ",512,-0.5,511.5,512,-0.5,511.5);
  hgemtrkr_peak_x = new TH1F("hgemtrkr_peak_x"," GEM-TRKR Peak X ; X [mm] ",512,-0.5,511.5);
  hgemtrkr_peak_y = new TH1F("hgemtrkr_peak_y"," GEM-TRKR Peak Y ; Y [mm] ",128,-0.5,127.5);
	hgemtrkr_2_peak_x = new TH1F("hgemtrkr_2_peak_x"," GEM-TRKR 2 Peak X ; X [mm] ",512,-0.5,511.5);
  hgemtrkr_2_peak_y = new TH1F("hgemtrkr_2_peak_y"," GEM-TRKR 2 Peak Y ; Y [mm] ",128,-0.5,127.5);
  
  //-- Clustering and NN track finding
  //-- Track fit in time
  gErrorIgnoreLevel = kBreak; // Suppress warning messages from empty chi^2 fit data
  TF1 fx1("fx1","pol1",40,150);
  f125_fit = new TH2F("f125_fit","GEM-TRD Track Fit; Time Response (8ns) ; Channel ",250,0.5,250.5,528,-0.5,527.5);
  ...
  //========================================================
  //        Create TTrees of Hit Info for NN (Rej Factor)
  //========================================================
  int gem_xch_max;
  int gem_ych_max;
  int gemtrkr_xch_max;
  int gemtrkr_ych_max;
	int gemtrkr_2_xch_max;
  int gemtrkr_2_ych_max;
  double gem_xamp_max;
  double gem_yamp_max;
  double gemtrkr_xamp_max;
  double gemtrkr_yamp_max;
	double gemtrkr_2_xamp_max;
  double gemtrkr_2_yamp_max;
  double chi2cc_gem;
  double gem_integral;
  double a0,a1;
  double a0nn,a1nn;
  ...
  TFile* fHits;
    ...
    //EVENT_VECT_GEM->Branch("xchtrkr2",&gemtrkr_2_xch_max);
    //EVENT_VECT_GEM->Branch("ychtrkr2",&gemtrkr_2_ych_max);
    //EVENT_VECT_GEM->Branch("xamptrkr",&gemtrkr_xamp_max);
    //EVENT_VECT_GEM->Branch("yamptrkr",&gemtrkr_yamp_max);
    //EVENT_VECT_GEM->Branch("xamptrkr2",&gemtrkr_2_xamp_max);
    //EVENT_VECT_GEM->Branch("yamptrkr2",&gemtrkr_2_yamp_max);
  ...
  for (jentry=FirstEvt; jentry<nentries; jentry++) {
    ...
    if (jentry<MAX_PRINT || !(jentry%1000)) {
      printf("------- evt=%llu  f125_raw_count=%llu f125_pulse_count=%llu f250_wraw_count=%llu, gem_scluster_count=%llu, gem_peak_count=%llu \n", jentry, f125_wraw_count, f125_pulse_count, f250_wraw_count, gem_scluster_count, gem_peak_count);
    }
    ...
    double gemtrkr_peak_height_y[gem_peak_count];
    double gemtrkr_peak_height_x[gem_peak_count];
    ...
    double gemtrkr_2_peak_height_y[gem_peak_count];
    double gemtrkr_2_peak_height_x[gem_peak_count];
    ...
    for (ULong64_t i=0;i<f125_pulse_count; i++) {
      ...
      if (gemChanX>-1 && amp>THRESH) {
        ...
        gem_xtime[nypulse]=time;
        nxpulse++;
      }
    } //---- End First Fadc125 Pulse Loop ----
    
    if(gem_xch_max>0&&gem_ych_max>0) {
      
      //---- Second Fadc125 Pulse Loop with the above condition----
      for (ULong64_t i=0;i<f125_pulse_count; i++) {
        ...
        if (gemChanX>-1 && amp>THRESH) {
          ...
          if (tcoin)f125_xamp2d->Fill(time,gemChanX,amp);
          f125_el_x->Fill(amp);
        }
    ...
    //===========================================================
    //  GEMTracker (SRS) Correlations
    //===========================================================
    
    ULong64_t gt_idx_x = 0, gt_idx_y=0, gt_2_idx_x=0, gt_2_idx_y=0;
    for (ULong64_t i=0; i<gem_peak_count; i++) { //-- SRS Peaks Loop
      //-- GEM Tracker 1
      if (gem_peak_plane_name->at(i) == "GEMTR1Y") {
        gemtrkr_peak_pos_y[gt_idx_y] = GetTrackerChan(gem_peak_index->at(i));
        gemtrkr_peak_height_y[gt_idx_y] = gem_peak_height->at(i);
        gt_idx_y++;
      } if (gem_peak_plane_name->at(i) == "GEMTR1X") {
        gemtrkr_peak_pos_x[gt_idx_x] = GetTrackerChan(gem_peak_index->at(i));
        gemtrkr_peak_height_x[gt_idx_x] = gem_peak_height->at(i);
        gt_idx_x++;
      }
      //-- GEM Tracker 2
      if (gem_peak_plane_name->at(i) == "GEMTR2Y") {
				gemtrkr_2_peak_pos_y[gt_2_idx_y] = GetTrackerChan(gem_peak_index->at(i));
        gemtrkr_2_peak_height_y[gt_2_idx_y] = gem_peak_height->at(i);
        gt_2_idx_y++;
			} if (gem_peak_plane_name->at(i) == "GEMTR2X") {
        gemtrkr_2_peak_pos_x[gt_2_idx_x] = GetTrackerChan(gem_peak_index->at(i));
        gemtrkr_2_peak_height_x[gt_2_idx_x] = gem_peak_height->at(i);
        gt_2_idx_x++;
			}
    } //-- End SRS Peak Loop
    ...
    gemtrkr_2_xch_max=-1.;
    gemtrkr_2_ych_max=-1.;
    gemtrkr_2_xamp_max=0.;
    gemtrkr_2_yamp_max=0.;
		
    if (gt_idx_x>0) {
      for (ULong64_t j=0; j<gt_idx_x; j++) {
        hgemtrkr_peak_x->Fill(gemtrkr_peak_pos_x[j]);
        if(gemtrkr_peak_height_x[j]>gemtrkr_xamp_max) {
          gemtrkr_xamp_max=gemtrkr_peak_height_x[j];
          gemtrkr_xch_max=gemtrkr_peak_pos_x[j];
        }
        if (gt_idx_y>0) {
        	...
        	for (ULong64_t i=0; i<gt_2_idx_x; i++) {
          	hgemtrkr_double_x->Fill(gemtrkr_peak_pos_x[j], gemtrkr_2_peak_pos_x[i]);
        	}
        }
      }
   	}
    ...
    if (gt_idx_y>0) {
      for (ULong64_t j=0; j<gt_idx_y; j++) {
        ...
        if(gemtrkr_peak_height_y[j]>gemtrkr_yamp_max){
          gemtrkr_yamp_max=gemtrkr_peak_height_y[j];
          gemtrkr_ych_max=gemtrkr_peak_pos_y[j];
        }
				if (gt_2_idx_y>0) {
					for (ULong64_t k=0; k<gt_2_idx_y; k++) {
						hgemtrkr_double_y->Fill(gemtrkr_peak_pos_y[j], gemtrkr_2_peak_pos_y[k]);
					}
				}
      }
    }
		...
		if (gt_2_idx_x>0) {
      for (ULong64_t j=0; j<gt_2_idx_x; j++) {
        hgemtrkr_2_peak_x->Fill(gemtrkr_2_peak_pos_x[j]);
        if(gemtrkr_2_peak_height_x[j]>gemtrkr_2_xamp_max) {
           gemtrkr_2_xamp_max=gemtrkr_2_peak_height_x[j];
           gemtrkr_2_xch_max=gemtrkr_2_peak_pos_x[j];
        }
        if (gt_2_idx_y>0) {
        for (ULong64_t k=0; k<gt_2_idx_y; k++) {
    ...
    if (gt_2_idx_y>0) {
      for (ULong64_t j=0; j<gt_2_idx_y; j++) {
        ...
        if(gemtrkr_2_peak_height_y[j]>gemtrkr_2_yamp_max) {
           gemtrkr_2_yamp_max=gemtrkr_2_peak_height_y[j];
           gemtrkr_2_ych_max=gemtrkr_2_peak_pos_y[j];
	      }
			}
		}
    
		if(gemtrkr_xamp_max>0) hgemtrkr_max_x->Fill(gemtrkr_xamp_max);
		if(gemtrkr_2_xamp_max>0) hgemtrkr_2_max_x->Fill(gemtrkr_2_xamp_max);
    ...
    //=========================================================================
      ...
      //=================================================================================================================
        ...
        //=================================== Draw HITS and CLUST  ========================================
        #ifdef SHOW_EVTbyEVT
          char hevtTitle[80]; sprintf(hevtTitle," GEM Display Event: %lld   Run: %d #Tracks: %d ; z pos,mm; y pos,mm ",jentry,RunNum, NTRACKS);
  ...
  const char *OutputDir="RootOutput/halld24";
    ...
    cc=NextPlot(nxd,nyd); hgemtrkr_max_x->Draw();
    cc=NextPlot(nxd,nyd); hgemtrkr_2_max_x->Draw();
